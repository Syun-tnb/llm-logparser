...
### 8.1 再取り込み（フルエクスポート前提）の挙動（MVP）

* 前提：プロバイダからのログエクスポートは毎回「全スレ全文」。
* スレ単位の軽量キャッシュを保持：
  * `update_time`（スレの最終更新時刻：エクスポート由来）
  * `last_message_id`（任意：ログ/診断用）
  * `count`（任意：将来のフォールバック用）
* 判定ルール（MVP）：
  * 新しい `update_time` が **前回と同じ** → **SKIP**（出力を再生成しない）
  * 新しい `update_time` が **前回より新しい** → **REPLACE**（当該スレ配下を削除し、現在の分割規則で**全文再生成**）
  * 新しい `update_time` が **前回より古い** → **WARN & SKIP**
    * 古いエクスポートファイルを誤って再パースした場合を想定
    * CLIログおよび `artifacts/logs/parser-warnings.log` に警告を出力（スレは保持）
* フォールバック（`update_time` が提供されない場合）：
  * **安全側ポリシー**として **REPLACE** に倒す（将来 `--safe-diff` 等のフラグで `count`/fingerprint を導入可）
* 非対象（MVP）：
  * `append-only` での末尾追記は行わない（真集合保証が難しいため）。必要になれば将来オプションで実装。
* 方針：
  * **元JSONには一切手を触れない**。取り込み失敗が疑われる場合はユーザー側で補完可能。
  * コミュニティ混乱を避けるため、曖昧な差分は **REPLACE** に統一する。
